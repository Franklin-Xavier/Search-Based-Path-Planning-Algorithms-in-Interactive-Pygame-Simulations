...
from .dfs import depth_first_search, reconstruct_path
from .bfs import breadth_first_search, reconstruct_path
from .bidirectional_astar import bidirectional_a_star_search, reconstruct_bidirectional_path
...
class Graph:
    def __init__(self, size, pos, algorithm=None):
        self.size = size
        self.position = pos
    ...
    def navigate(self):
        ...
        if self.algorithm == 'BFS':
          # calculate paths for all agents
            for ag in self.agents:
                ...
                cf = breadth_first_search(self, start, goal)
                try:
            ...
            for ag in self.agents:
                ...
                cf = depth_first_search(self, start, goal)
                try:
                    path = reconstruct_path(cf, start, goal)
                except KeyError:
                        return
        ...
        elif self.algorithm == 'BiAStar':
            for ag in self.agents:
                start = ag.rect.center
                goal = self.target

                forward_came_from, backward_came_from, intersection_node = bidirectional_a_star_search(self, start, goal)
                try:
                    path = reconstruct_bidirectional_path(forward_came_from, backward_came_from, start, goal, intersection_node)
                except KeyError:
                    return
